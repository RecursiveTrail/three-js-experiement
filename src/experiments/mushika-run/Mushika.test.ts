...
import { frontPawFrame, groundBlobVisible } from './Mushika'

describe('frontPawFrame', () => {
  it('raises paws to chest and in front of the belly during shrine', () => {
    const left = frontPawFrame({ shrine: true, hopping: false, stride: 0, side: 1 })
    const right = frontPawFrame({ shrine: true, hopping: false, stride: 0, side: -1 })
